...
export class PushUtils {
  ...
  static validateMessage(message: any): message is WebSocketMessage {
    ...
    // 心跳消息的特殊格式验证
    const isHeartbeatMessage = message.messageType === 'HEARTBEAT' || 
    ...
    if (multiLanguage) {
      ...
      for (const key of possibleKeys) {
        if (multiLanguage[key]) {
          return {
            title: multiLanguage[key].title,
            content: multiLanguage[key].content
          }
        }
      }
      ...
      if (availableKeys.length > 0) {
        ...
        return {
          title: multiLanguage[firstKey].title,
          content: multiLanguage[firstKey].content
        }
      }
    }
