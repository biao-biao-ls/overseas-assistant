...
export class MessageProcessor {
  ...
  private initializeMessageProcessor(): void {
    try {
      ...
      this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'initializeMessageProcessor', '消息处理器初始化失败', error)
  ...
  private registerDefaultHandlers(): void {
    ...
    this.registerHandler(MessageType.NOTIFICATION, async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleNotificationMessage', '处理通知消息失败', error)
        throw error
      }
    })
    ...
    this.registerHandler(MessageType.HEARTBEAT, async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleHeartbeatMessage', '处理心跳消息失败', error)
        throw error
      }
    })
    ...
    this.registerHandler(MessageType.HEARTBEAT_RESPONSE, async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleHeartbeatResponseMessage', '处理心跳响应消息失败', error)
        throw error
      }
    })
    ...
    this.registerHandler(MessageType.ACK, async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleAckMessage', '处理 ACK 消息失败', error)
        throw error
      }
    })
    ...
    this.registerHandler(MessageType.READ_RECEIPT, async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleReadReceiptMessage', '处理 READ_RECEIPT 消息失败', error)
        throw error
      }
    })
    ...
    this.registerHandler('CONNECTION_ACK', async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleConnectionAckMessage', '处理连接确认消息失败', error)
        throw error
      }
    })
    ...
    this.registerHandler('SYSTEM_MESSAGE', async (message) => {
      try {
        ...
        this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'handleSystemMessage', '处理系统消息失败', error)
        throw error
      }
    })
  }
  ...
  private parseMessage(rawMessage: string): ParsedMessage | null {
    try {
      ...
      if (!PushUtils.validateMessage(messageObj)) {
        this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'parseMessage', '消息格式验证失败，但尝试继续解析', {
  ...
  private validateMessage(message: ParsedMessage): boolean {
    try {
      ...
      // 心跳消息的宽松验证
      const isHeartbeatMessage = message.messageType === MessageType.HEARTBEAT || 
      ...
      const isSystemMessage = message.messageType === 'CONNECTION_ACK' ||
      ...
      if (typeof message.priority !== 'number') {
        this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateMessage', '消息优先级无效，使用默认值')
        message.priority = 0 // 设置默认优先级
      }
      
      if (typeof message.timestamp !== 'number') {
        this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateMessage', '消息时间戳无效，使用当前时间')
      ...
      if (timeDiff > 24 * 60 * 60 * 1000) { // 24小时
        this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateMessage', '消息时间戳异常，但继续处理', {
          timestamp: message.timestamp,
          timeDiff: timeDiff
        })
      }
  ...
  private validateMessageByType(message: ParsedMessage): boolean {
    switch (message.messageType) {
      ...
      default:
        // 未知消息类型，记录警告但不拒绝
        this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateMessageByType', '未知消息类型，但允许处理', {
          messageType: message.messageType
        })
        return true
    }
  ...
  private validateNotificationMessage(message: ParsedMessage): boolean {
    ...
    if (!data.messageId || typeof data.messageId !== 'string') {
      this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateNotificationMessage', '通知消息缺少消息ID')
      return false
    }
    ...
    if (!hasEnglishContent && !hasMultiLanguageContent) {
      this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateNotificationMessage', '通知消息缺少内容')
      return false
    }
    
    // 验证 URL 格式（如果存在）- 宽松验证，不阻止消息处理
    if (data.url && !PushUtils.isValidUrl(data.url)) {
      this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'validateNotificationMessage', '通知消息URL格式可能无效，但继续处理', {
        url: data.url
      })
      // 不返回 false，继续处理消息
    }
  ...
  private addToQueue(message: ParsedMessage): void {
    ...
    if (this.messageQueue.length >= MESSAGE_CONSTANTS.QUEUE_MAX_SIZE) {
      this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'addToQueue', '消息队列已满，丢弃最旧的消息')
    ...
    try {
      ...
      for (const message of messagesToProcess) {
        try {
          ...
          this.failedMessages++
          this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'processMessageQueue', '处理消息失败', error)
        }
      }
      ...
      this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'processMessageQueue', '处理消息队列失败', error)
    } finally {
  ...
  private async routeMessage(message: ParsedMessage): Promise<void> {
    try {
      ...
      if (!handlers || handlers.length === 0) {
        this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'routeMessage', '没有找到消息处理器', {
          messageType: message.messageType
        })
        return
      }
  ...
  private async executeHandler(handler: MessageHandler, message: ParsedMessage): Promise<void> {
    try {
      ...
      this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'executeHandler', '执行消息处理器失败', error)
      throw error
    }
  }
  ...
  registerHandler(messageType: string, handler: MessageHandler): void {
    try {
      ...
      this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'registerHandler', '注册消息处理器失败', error)
      throw error
    }
  }
  ...
  removeHandler(messageType: string, handler: MessageHandler): void {
    try {
      ...
      this.logger.error(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'removeHandler', '移除消息处理器失败', error)
    }
  }
  ...
  private getLocalizedContent(data: any): { title: string; content: string } {
    try {
      // 使用工具方法获取本地化文本
      return PushUtils.getLocalizedText(
        data.multiLanguage,
        data.titleEn,
        data.contentEn
      )
    } catch (error) {
      this.logger.warn(LOG_CONSTANTS.COMPONENT_NAMES.MESSAGE_PROCESSOR, 'getLocalizedContent', '获取本地化内容失败', error)
  ...
  getMessageStatistics(): any {
    ...
    return {
      ...
      formattedLastProcessTime: this.lastProcessTime > 0 ? PushUtils.formatTimestamp(this.lastProcessTime) : '从未处理',
      ...
      successRate: this.totalMessages > 0 ? (this.processedMessages / this.totalMessages * 100).toFixed(2) + '%' : '0%',
      failureRate: this.totalMessages > 0 ? (this.failedMessages / this.totalMessages * 100).toFixed(2) + '%' : '0%'
    }
  }
